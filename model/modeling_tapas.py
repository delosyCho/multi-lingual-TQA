...
def is_torch_greater_or_equal_than_1_12():
...
class TapasForQuestionAnswering(TapasPreTrainedModel):
    ...
    def forward(
        ...
        if not return_dict:
            output = (logits, logits_aggregation) + sequence_output
...
def _single_column_cell_selection_loss(token_logits, column_logits, labels, cell_index, col_index, cell_mask):
    ...
    # print(column_label[:], column_logits[:, :7])
    ...
    cell_loss = -torch.sum(cell_log_prob * column_mask, dim=1)

    # We need to normalize the loss by the number of cells in the column.
    cell_loss /= torch.sum(column_mask, dim=1) + EPSILON_ZERO_DIVISION
